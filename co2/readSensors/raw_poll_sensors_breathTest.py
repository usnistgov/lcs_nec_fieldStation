...
'''
Specific version used for breath tests and timestamp synchronization
- measurement timestamp is specifically appended to variable
...
This script must be in the same directory as fn_k30.py and fn_k96.py
	...
	Modified: T.P. Boyle 05/2025 - added in timestamp appendending to file
...
if(len(k96_1_response) > 1):				# If sensor reading is returned
	print(*k96_1_response, "epoch",int(time.time()))				# print response to stdout for redis program
...
if(len(k96_2_response) > 1):                            # If sensor reading is returned
        print(*k96_2_response, "epoch",int(time.time()))                          # print response to stdout for redis program
